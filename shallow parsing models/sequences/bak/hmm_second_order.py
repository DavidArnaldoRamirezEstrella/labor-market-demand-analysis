...
from bak.viterbi import viterbi
#from bak.forward_backward import forward_backward, sanity_check_forward_backward



class HMM_Second_Order():
    ''' Implements an HMM with second order dependecies
    We will use the state expansion version where we expand the states to be n^2 and use the normal infernece algorithms
    ...
    def __init__(self, dataset, nr_states=-1):
        ...
        if(nr_states == -1):
            self.nr_states = len(dataset.ne_dict)
        else:
            self.nr_states = nr_states
        self.nr_types = len(dataset.word_dict)
    ...
    def train_supervised(self,sequence_list, smoothing=0):
        if(len(self.dataset.ne_dict) != self.nr_states):
    ...
    def print_transitions(self,table):
        txt = "\t"
        for prev_state in range(self.nr_states+1):
            for prev_prev_state in range(self.nr_states+1):
                txt += "%i-%i\t"%(prev_state,prev_prev_state)
        print(txt)
        
        for state in range(self.nr_states+1):
            txt = "%i\t"%state
            for prev_state in range(self.nr_states+1):
                for prev_prev_state in range(self.nr_states+1):
    ...
    def print_observations(self,table):
        txt = "\t"
        for obs in range(self.nr_types+1):
            txt += "%i\t"%(obs)
        print(txt)
    ...
    def collect_counts_from_corpus(self,sequence_list):
        ...
        for sequence in sequence_list.seq_list:
            ...
            for pos in range(len_x+self.order):
                if(pos >= len_x):

...
class SequenceClassifier():
    ...
    def compute_posteriors(self, initial_scores, transition_scores,
        ...
        #state_posteriors[length-1, :] = forward[length-1, :] - log_likelihood
    ...
    def viterbi_decode(self, sequence):
        ...
        for state in best_states:
            y_name = self.y_dict.get_label_name(state)
            y_trans = sequence.sequence_list.y_dict.get_label_id(y_name)
            translated.append(y_trans)
        predicted_sequence = sequence.copy_sequence()
        predicted_sequence.y = translated
        return predicted_sequence, total_score


    def viterbi_decode_bigram(self, sequence):
        '''Compute the most likely sequence of states given the observations,
        by running the Viterbi algorithm.'''

        # Compute scores given the observation sequence.
        initial_scores, transition_scores, final_scores, emission_scores = \
            self.compute_scores_bigram(sequence)

        if self.counter % 1000 == 0:
            print("DEBUG: viterbi_decode_bigram: computed_scores! + %i" % self.counter)
        self.counter += 1

        # Run the forward algorithm.
        best_states, total_score = self.decoder.run_viterbi_bigram(initial_scores,
                                                                          transition_scores,
                                                                          final_scores,
                                                                          emission_scores, sequence)
        
        """
        #   NO ES NECESARIO!!! DE HECHO ESTA MAL LEL
        translated = []
    ...
    def viterbi_decode_trigram(self, sequence):
        '''Compute the most likely sequence of states given the observations,
        by running the Viterbi algorithm.'''

        # Compute scores given the observation sequence.
        initial_scores, transition_scores, final_scores, emission_scores = self.compute_scores_trigram(sequence)

        # Run the forward algorithm.
        best_states, total_score = self.decoder.run_viterbi(initial_scores, transition_scores, final_scores,
                                                            emission_scores)
    ...
    def viterbi_decode_corpus_bigram(self, dataset):
        '''Run viterbi_decode at corpus level.'''
    ...
    def viterbi_decode_corpus_trigram(self, dataset):
        '''Run viterbi_decode at corpus level.'''

        predictions = []
        for sequence in dataset.seq_list:
            predicted_sequence, _ = self.viterbi_decode_trigram(sequence)
            predictions.append(predicted_sequence)
        return predictions
    ...
    def evaluate_corpus(self, dataset, predictions, DEBUG=False):
        ...
        :param predictions: lista de sequence objects
        '''
        total = 0.0
        correct = 0.0
        for i, sequence in enumerate(dataset.seq_list):
            ...
            for j, y_hat in enumerate(pred.y):
                if (sequence.y[j] == y_hat):
                    correct += 1
                total += 1
            if DEBUG and any(sequence.y != pred.y):
                print("#############################################")
                print(pred)
                print("position: ",i)
                ipdb.set_trace()

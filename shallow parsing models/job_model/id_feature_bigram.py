...
class IDFeatures:
    ...
    def get_inner_list_features(self,sequence,pos,y_name,include_stem=False,features=[]):
        ...
        w_prev,_,_,_  = self.get_label_names(sequence,pos-1)
        w,_,_,stem  = self.get_label_names(sequence,pos, escape_ascii=True)
        w_next = ''
        if pos < length-1:
            w_next,_,_,_  = self.get_label_names(sequence,pos+1)

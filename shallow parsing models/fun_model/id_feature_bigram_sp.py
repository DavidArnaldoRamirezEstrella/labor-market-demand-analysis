...
ORT = [
    ...
    re.compile(r'^[a-zA-Z]{,%i}$' % SHORT_WORD_THR, re.UNICODE),        # SHORT WORD <= THRESHOLD
...
def unwrapp_seq_idf(arg, **kwargs):
    return IDFeatures.get_sequence_features(*arg,**kwargs)
...
class IDFeatures:
    ...
    def build_features(self):
        ...
        features = pool.map(unwrapp_seq_idf,zip([self]*len(self.dataset.seq_list),self.dataset.seq_list))
        pool.close()
        pool.join()
    ...
    def get_isverb_features(self,low_word,y_name,pos,features=[]):
        suffixes = self.get_suffixes(low_word)
        found=False
        for suf in suffixes:
            if suf in ['ar','er','ir','ado','ido']:
                found=True
                break
        if found:
            feat_name = 'isverb::%i::%s' % (pos,y_name)
            features = self.insert_feature(feat_name,features)

        return features

    def get_isaction_features(self,low_word,y_name,pos,features=[]):
        suffixes = self.get_suffixes(low_word)
        found=False
        for suf in suffixes:
            if suf in ['cion','ción','tion']:
                found=True
                break
        if found:
            feat_name = 'isaction::%i::%s' % (pos,y_name)
            features = self.insert_feature(feat_name,features)
        return features

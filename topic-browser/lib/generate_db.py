"""
run:
python2 lib/generate_db.py NE_ene_ing/job_db ../dataR/counts/NE_ene_ing/word_counts.dat ../dataR/counts/NE_ene_ing/beta_Gibbs26 ../dataR/counts/NE_ene_ing/gammaGibbs1000 ../dataR/counts/NE_ene_ing/vocab.dat ../dataR/counts/NE_ene_ing/filtered_title_map.dat

if testing with 10 docs:                                                                                                                                           ""_10                                                                        ""_10.dat
...
def write_topics(con, cur, beta_file, vocab):
	...
	indices = range(len(topic))
        ...
        cur.execute('INSERT INTO topics (id, title) VALUES(NULL, ?)', ["{" + vocab[indices[0]] + ', ' + vocab[indices[1]] + ', ' + vocab[indices[2]] + '}'])
...
def write_terms(con, cur, terms_file):
    ...
    for line in io.open(terms_file, 'r',encoding='utf-8'):
        line = line.strip('\n').replace('<','.').replace('>','.').replace('/','_slash_')
        cur.execute('INSERT INTO terms (id, title) VALUES(NULL, ?)', [line])
...
def write_docs(con, cur, docs_file):
    ...
    for line in io.open(docs_file, 'r'):
...
if (__name__ == '__main__'):
    ...
    vocab = io.open(vocab_file, 'r',encoding='utf-8').readlines()

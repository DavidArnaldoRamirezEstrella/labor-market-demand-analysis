...
Ainner = '|'.join(Ainner)
bett = '|'.join(bet)
#                        Ainner \s+,? (<bet>\s+)?,? carrera
patternInner = '(%s)\\s*,?((%s)\\s+)?[,:;]?\\s*(?P<carreras>.*)' % (Ainner,bett)
...
def compilePatterns():
    ...
    pattern2 = '(%s)\\s+(%s)*\\s*((%s)\\s+(%s))*\\s*((%s)\\s+)?(?P<carreras>.*).?' % (A,C,sep,C,bett)
    ...
    pattern45 ='(%s)\\s*((%s)\\s+((%s)\\s+(%s))*)*\\s*(:|;).*\\b(%s)\\b\\s*(?P<carreras>.*).?' % (Amultiword,C,sep,C,bett)
    pattern45 = re.compile(pattern45)

    ### Quinto:                  6
    #** <B> sf <B>? <A> <C>? (o C)? <de,en,->
    #            carreras        \s+  A \s* ( C   \s ( o   \s  C )* )? (\s+ bet)?
    pattern5 ='^(?P<carreras>.*)\\s+(%s)\\s*((%s)\\s*((%s)\\s+(%s))*)?(\\s+(%s))?' % (A_post,C,sep,C,bett)
...
TEC_TH = 6  # umbral para patrones de tecnico : last normal index
...
def careersFromPatterns(text,patterns,index,debug=False):
    ...
    for i,car in enumerate(careers):
        ...
        try:
            ...
            t = fin.sub('',t)
            if '(' in t or ')' in t:
                rep_parenthesis = t.replace('(',',').replace(')',',')
                rep_parenthesis = rep_parenthesis.replace(',,',',').replace(', ,',',').replace(' , ',', ')
            t = clean_parentesis.sub('',t).strip(' ').replace(' , ',', ')
        except:
            pass

        # no agregar si no hay ningun parentesis
        if stemAugmented(t) not in stopwords or t=='tecnico': # caso especial : tecnico en smartlist
            temp.append(t)          # igual lo tiene que agregar
            if i==n-1 and added:    # si es el ultimo y tuvo parentesis, guardar indice
                idx_agreg.append(len(temp)-1)
        
        if rep_parenthesis != '' and rep_parenthesis!=text:
            temp.append(rep_parenthesis)
            if i==n-1 and added:
                idx_agreg.append(len(temp)-1)
        """
    careers = list(temp)
